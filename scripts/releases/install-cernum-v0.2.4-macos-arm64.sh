#!/bin/bash
# Cernum v0.2.4 — MacBook Pro (Apple Silicon): upgrade, verify the v0.2.4 corrections, spend nothing.
#
...
#
# IT SENDS NO PROVIDER REQUEST. No smoke test, no campaign, no model invoked. Step 16 checks the
# v0.2.4 corrections through `--dry-run` and `help`, both of which are free, and the OpenCode dry-run
# it runs is one this build would REFUSE to send without an explicit spending authorization.
...
if grep -q "no OpenCode execution adapter" "$DISCOVERY_OUT"; then
  die "discovery still carries the v0.2.2 no-proof-path sentence. This is not a v0.2.4 build."
fi
...
step "16. The v0.2.4 corrections, checked without sending anything"
...
if [ "$HELP_SMOKE" = "$HELP_FLAG" ] && [ "$HELP_SMOKE" = "$HELP_SHORT" ]; then
  ...
  die "the three help spellings disagree. This is not a v0.2.4 build."
fi
for flag in --models --all-ladder --dry-run --pricing --authorize-metered --authorize-unpriced-metered --otlp-observer; do
  case "$HELP_SMOKE" in *"$flag"*) ;; *) die "\`cernum help smoke\` does not document ${flag}. This is not a v0.2.4 build." ;; esac
done
...
case "$CLAUDE_PREVIEW" in
  ...
  *) die "the Claude preview does not name its billing basis. This is not a v0.2.4 build." ;;
esac
...
case "$OPENCODE_PREVIEW" in
  ...
  *) die "the OpenCode preview does not name meteredAPI. This is not a v0.2.4 build." ;;
esac
case "$OPENCODE_PREVIEW" in
...
cat <<SUMMARY
   ...
   v0.2.4 checks: help is one page across all three spellings; malformed options refuse;
