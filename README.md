...
| **[OpenCode](https://opencode.ai)**, via the official `opencode` CLI | ⚠️ Executable, never yet run live | New in v0.2.3: Cernum can send OpenCode a request, with the model named explicitly in OpenCode's own `provider/model` addressing, in an isolated empty working directory. It captures tokens, cost, latency, retries and finish reason where OpenCode reports them and marks them unavailable where it does not. **A catalogue listing still proves nothing** — `opencode models` is read from a locally cached catalogue, so every model starts discovered and UNPROVEN and only an authorized request that came back can prove one. Classed as a **metered API**: billed per token, and its cost is never reported as subscription allowance. **No Cernum request has ever been sent to OpenCode**, and no OpenCode result is published in this repository. |
