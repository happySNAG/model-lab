...
Run models through repeatable, deterministic evaluations — the ones on your own computer, and the
frontier models you already pay for. Compare quality, speed, reliability and safety behaviour side by
side, and pick the right model for each job with evidence instead of vibes.

[![License: MIT](https://img.shields.io/badge/License-MIT-informational.svg)](LICENSE)
[![Platform](https://img.shields.io/badge/platform-macOS%20%7C%20Windows-lightgrey.svg)](#install)
[![Runtime](https://img.shields.io/badge/runtimes-Ollama%20%C2%B7%20Claude%20CLI%20%C2%B7%20Codex%20CLI-black.svg)](#supported-models-and-runtimes)
[![Tests](https://img.shields.io/badge/tests-1307%20unit%20%2B%20parity%20%C2%B7%20end--to--end-brightgreen.svg)](#tests)
[![Status](https://img.shields.io/badge/status-v0.2.0%20Cernum-blue.svg)](#project-status)
...
Cernum is a native desktop application. You pick some models, pick what to test, press start, and
...
**Test.** The same prompts, the same budgets, the same judge, every time.
**Discern.** Every number arrives with what it does *not* prove: whether the model's identity was
verified, what a cost figure actually covers, how wide the interval is, what was withheld for a
person to rule on.
**Decide.** That part is yours. Cernum ranks candidates *within one benchmark* and hands you the
evidence. It never routes, promotes or recommends a model on its own authority.

Cernum has no account, no telemetry, no crash reporting and no update check. Local models run on your
machine and their prompts never leave it. If you add a frontier candidate, that candidate's prompts go
to that provider — Cernum names which one, and asks you once, before the campaign starts.

**Quick start:** install [Ollama](https://ollama.com/download) → download Cernum → open it → press
...
| **Windows 10 / 11** (x64) | Builds from the same source: the installer and portable executable are produced and verified. **Nothing has yet been run on a Windows machine** — not the test suites, not the application. Read [Windows verification status](#windows-verification-status) before relying on it. |
...
Cernum answers those questions instead:
...
**The suites are fixed, versioned fixtures.** Cernum ships 14 suites and 44 cases: a short
*Quick check* (11 attempts per model), the *Full lab* (46 attempts per model), or any hand-picked
subset. Every prompt is an invented synthetic fixture — Cernum reads no files of yours, no
...
not report your GPU, Cernum says so instead of inferring one.
...
- **Rankings apply within one benchmark only.** Every model in it faced the same prompts on the same
  machine. Cernum never produces a universal score and never compares against numbers from
  somewhere else.
...
| --- | --- | --- |
| **[Ollama](https://ollama.com)**, on this computer | ✅ Yes | Any model you have installed — `llama3.2:3b`, `gemma3:4b`, `qwen3:4b`, and so on. Cernum detects Ollama, can start it for you, and can ask it to download a model after you confirm by name. The endpoint is configurable, so any Ollama-compatible server on a loopback address works, including one on a non-default port such as `11435` — which is how **Muse** builds are reached. Muse is not a separate provider or adapter in Cernum: it is local models served over this transport. |
| **Built-in reference model** | ✅ Yes, always | A deterministic scripted responder built into the application. Same fixed answers every time. It exists so you can try the entire workflow — and check that a change to Cernum itself did not change any judgment — without installing anything. Its answers are not a real model's. |
...
| **[OpenCode](https://opencode.ai)**, via the official `opencode` CLI | ⚠️ Executable, never yet run live | Cernum can send OpenCode a request, with the model named explicitly in OpenCode's own `provider/model` addressing, in an isolated empty working directory. It captures tokens, latency, retries and identity where OpenCode reports them and marks them unavailable where it does not. **A catalogue listing proves nothing** — `opencode models` is read from a locally cached catalogue, so every model starts discovered and UNPROVEN and only an authorized request that came back can prove one. Classed as a **metered API**: billed per token, its cost is never reported as subscription allowance, and from v0.2.4 a live OpenCode request is **refused without an explicit spending authorization** — a ceiling where you supply prices, or a written acknowledgement of an unknown amount for exactly one request. Where Cernum holds no price, the cost is recorded as UNAVAILABLE and never as zero. **No Cernum request has ever been sent to OpenCode**, and no OpenCode result is published in this repository. |
...
- **Cernum itself phones nobody.** No telemetry, no crash reporting, no update check, no account, no
  analytics — ever, including when you use a frontier candidate.
- **Local candidates stay local.** For a model served by Ollama on loopback, the text of a benchmark
  never reaches a third party. That is the point of them.
- **Frontier candidates do not, and Cernum says so before it starts.** If you add a Claude, Codex,
  OpenCode or hosted-API candidate, then for *that* candidate every benchmark prompt, every supplied
  context and every answer leaves this machine and is processed on the provider's systems, under the
  provider's terms and retention policy — not Cernum's. Cernum cannot recall a prompt once sent and
  cannot make a provider forget one. You are asked once, before the campaign runs, and not again
  between attempts. Local candidates in the same campaign are unaffected.
- **No personal data is read.** Every prompt is a fixed synthetic fixture shipped with the
  application. Cernum does not read your files, messages, contacts or calendar, and has no code
  that could.
...
- **No credential is ever written down.** Cernum reads an API key from your environment or keychain at
  the moment of use. It is never written to disk, never stored in a campaign, a manifest, a log or an
  evidence bundle, and never printed. For the subscription CLIs there is no key at all: the tool holds
  its own session and Cernum never reads it.
- **Reasoning traces are never stored.** If a model produces internal "thinking", only its length is
  recorded.
- **Downloads are explicit.** Cernum never installs Ollama and never pulls a model on its own. The
...
| **Ollama** | Free, from [ollama.com/download](https://ollama.com/download). Cernum detects it, can start it, and guides you if it is missing — but never installs it for you. |
...
1. Download `Cernum-<version>-macos-arm64.dmg` (Apple Silicon) or
   `Cernum-<version>-macos-x64.dmg` (Intel) from the [releases page][releases]. Not sure which Mac
   you have? Apple menu → *About This Mac*: "Chip: Apple M…" means arm64.
2. Open the disk image and drag **Cernum** into **Applications**.
...
the internet is quarantined and the first launch shows *"Cernum" Not Opened*. This is expected:
...
- **macOS 12–14:** right-click (Control-click) **Cernum** in Applications and choose **Open**, then
  **Open** again.
- Or clear the quarantine flag yourself:
  `xattr -d com.apple.quarantine "/Applications/Cernum.app"`
...
1. Download `Cernum-Setup-<version>-x64.exe` and run it. It installs per user (no administrator
   rights), adds Start Menu and Desktop shortcuts, and starts Cernum when it finishes. Or download
   `Cernum-Portable-<version>-x64.exe` and run it from anywhere with no installation.
...
| **Campaigns** | A benchmark made durable. Freezes exactly what will be run, records each attempt to disk as it lands, survives an interruption, and resumes without losing or repeating one. Verify what was frozen, finalize into rankings and retention notes, and open the blinded packet for anything a person still has to judge. Drivable from here or from the `cernum` terminal command, which ships inside the installed application and can be installed for your account from this screen — one file, no PATH change. One runner owns a campaign at a time and one campaign owns a benchmark endpoint at a time, across both. For a canonical campaign Cernum manages model residency on the selected endpoint — disclosed before it starts, and on that endpoint only — or you can choose observe-only, which is labelled noncanonical everywhere it appears. See [docs/ENGINE.md](docs/ENGINE.md). |
| **History** | Every benchmark ever run on this computer. Reopen any result; tick two to compare them — Cernum says plainly when they are not like-for-like. |
...
1. Open Cernum. The Home screen tells you whether Ollama is running and offers to start it.
...
**"Ollama is not installed" but I installed it.** Cernum looks for the `ollama` executable in the
...
- **Frontier support is uneven, and the table says which is which.** Claude and Codex subscription
  CLIs have been exercised through full scored campaigns. OpenCode is *discovery only* — Cernum can see
  it and address its models, but no scored campaign has ever run through it. The Anthropic and OpenAI
  key-based paths are implemented and tested but have never been used in a published campaign. See
  [Supported models and runtimes](#supported-models-and-runtimes).
- **A frontier candidate's numbers mean less than a local one's.** Latency and throughput for a hosted
  model measure a network and somebody else's fleet on the day you asked, not a machine you control,
  and they are not reproducible. Cernum records them and labels the execution class on every row so
  the two are never averaged together by accident.
- **Cost is reported only where it is known.** A subscription request has a zero *marginal* charge
  against a finite allowance, which Cernum reports as `subscriptionIncluded` and never as free. Where
  there is no pricing to read — OpenCode is the current example — the cost is reported as
  `unavailable`, not estimated and not zeroed.
...
| | |
| --- | --- |
| The Windows installer and portable executable build | ✅ **Verified** — cross-built from macOS; both artifacts produced, with the installer licence page configured |
| The engine is platform-independent TypeScript with no Windows-specific logic | ✅ **Verified** by reading it — nothing in `src/core/` touches an operating-system API |
| Typecheck, the 200 unit and parity tests, and the production build, on Windows | ⏳ **Not yet run.** `.github/workflows/ci.yml` is configured to run them on `windows-latest` on every push, but no CI run has happened — this repository has not been pushed anywhere yet |
| The end-to-end suite driving the real Electron application on Windows | ⏳ **Not yet run.** Also configured in CI, also never executed |
| Windows-specific code paths (CIM hardware query, `%APPDATA%` locations, Ctrl accelerators, starting Ollama) | ⚠️ **Reviewed by reading the code only.** Never executed anywhere |
| Installing, launching and uninstalling the packaged application on a real Windows PC | ❌ **Not done** |

The honest summary: Windows support is *written*, and its artifacts *build*. It has not been
*observed*. The first CI run on this repository closes the two ⏳ rows on its own; the ❌ row needs a
person with a Windows PC.

`scripts/windows-smoke.ps1` performs that last check — silent install, shortcut and registry checks,
launch, evidence-store check, uninstall. It has never been run. If you run it, please
...
npm run dist:mac       # Cernum.app + DMG + ZIP, arm64 and x64 → dist/
...
src/engine/     the benchmark engine: frozen manifests, the durable ledger, safety guards,
                residency proof, supplied-context and model-identity verification, per-attempt
                telemetry, isolated execution, blinded adjudication, rankings, roles, retention.
                One boundary, shared by the desktop application and the terminal command.
src/cli/        cernum — the supported terminal interface, bundled into the application
src/main/       Electron main process: window, macOS/Windows menu, environment capture, Ollama
                detection / start / explicit pull, sessions, settings, the lab service, the
                campaign service, IPC
src/preload/    the narrow typed bridge the renderer may call
src/shared/     ipc.ts — the single contract between main and renderer
src/renderer/   the React interface (Home, Models, Benchmark, Live run, Results, Campaigns,
                History, Settings)
...
| `test/engine/` (276) | The benchmark engine: byte-for-byte canonical-encoding parity with the Python harness this engine is ported from, the durable ledger (torn writes, checkpoint drift, duplicate results, resume, reconciliation), frozen-manifest drift detection, new-hardware retest derivation, the safety guards, the residency proof, supplied-context and model-identity verification, per-attempt telemetry from an observed token stream (visible-only, thinking-then-visible, thinking-only budget exhaustion, malformed stream, mid-stream disconnect, cancellation and resume) and its refusal to estimate a token count it was not given, the isolation boundary, blinded packets and their leak audit, rankings and capability roles, retention interpretation, the cross-process campaign lock and the runtime-endpoint lease (atomic acquisition raced between real processes, live refusal, stale recovery after a crash, never breaking a live lock, endpoint normalization, and two campaigns on separate endpoints running side by side), canonical versus observe-only execution (residency managed and proved, residency failure as an abort rather than a warning, and an observe-only run labelled noncanonical in its manifest identity, every ledger row, its rankings, its counting rules and its retention notes), frozen thinking mode and its preflight refusal, the packaged terminal launcher and the one file its install writes, and a complete synthetic campaign driven end to end — created, paused, resumed, aborted, finalized and reviewed — without a request reaching any server. |
| `test/e2e/` (15) | The built application under Playwright: install-to-results with the reference model, relaunch persistence, cancel-and-keep, a two-model benchmark against a scripted Ollama covering HTTP failure, malformed output, timeout and a hard-boundary violation, settings persistence, refusal of non-loopback endpoints, menu navigation, the packaged `Cernum.app` launching from its bundle, one campaign created, paused and resumed entirely in the terminal then read, verified and finalized in the desktop application, one campaign contested by two real processes — a second runner refused by name, the holder shown on the Campaigns screen, and the lock reclaimed after the holder is killed outright; and, against a scripted streaming runtime, a canonical campaign started from the desktop Start button running to clean completion, a runtime that will not release its weights aborting the campaign, an observe-only campaign labelled noncanonical throughout, a second campaign on the same endpoint refused before a single request, and the terminal refusing to change a mode the manifest froze. |

`.github/workflows/ci.yml` is configured to run the typecheck, unit and parity suites on macOS,
Windows and Linux, and the end-to-end suite on macOS and Windows, on every push and pull request. The
counts above were verified on macOS; the Windows and Linux legs will report for the first time on the
first push.
...
[MIT](LICENSE). Third-party components and the licences of the runtimes and models Cernum talks to
