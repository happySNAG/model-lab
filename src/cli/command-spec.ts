...
export const COMMAND_SPECS: CommandSpec[] = [
  ...
  },
  {
    ...
    detail: [
      ...
      '  A codexCLI or opencodeCLI request that is accepted and answered lands on',
      '  requestAcceptedIdentityUnverifiable: `codex exec` names no model in any reply, and `opencode',
      '  run --format json` emits no assistant message, so such a smoke can establish that the provider',
  ...
  // The fleet: what each machine observed, how it travels between them, and what it makes routable.
  {
    name: 'observations', effect: 'readOnly', positional: '',
    summary: 'what this machine has observed, and what it has imported from others',
    options: [...ROOT, { name: 'json', takesValue: false, summary: 'print the store contents as JSON' }],
    detail: [
      'The observation store is APPEND-ONLY and content-addressed: a record is named by the digest of',
      'its own content, so the same observation is one file however many times it is imported.',
      '',
      'Every record carries the machine that OBSERVED it. A record imported from another machine keeps',
      'that machine\'s key, and this machine never reads it as a local fact: availability is always',
      'answered from observations taken on the machine being asked about.',
      '',
      'A file whose bytes no longer digest to its own name is reported here, by path, and is not read.',
    ],
  },
  {
    name: 'observations-export', effect: 'writesLocal', positional: '',
    summary: 'write this machine\'s observations to a transport-neutral bundle',
    options: [...ROOT,
      { name: 'out', takesValue: true, summary: 'the bundle file to write. Required' },
      { name: 'machine', takesValue: true, summary: 'only observations made on this cmk1: key. Repeatable by comma' },
      { name: 'since', takesValue: true, summary: 'only observations made at or after this ISO-8601 instant' }],
    supportsDryRun: true,
    detail: [
      'The bundle is ONE JSON value, sealed with a digest over its whole content. Nothing in it knows',
      'how it travels: copy it over a shared folder, a USB stick, scp, or anything else. There is no',
      'daemon, no protocol and no hard-coded host or path.',
      '',
      'Records are sorted by their own identity, so exporting the same set twice produces the same',
      'bytes and the same bundle digest — two machines can tell whether they hold the same thing.',
    ],
  },
  {
    name: 'observations-import', effect: 'writesLocal', positional: '<bundle>',
    summary: 'import another machine\'s observation bundle, idempotently',
    options: [...ROOT],
    supportsDryRun: true,
    detail: [
      'The bundle\'s digest is checked BEFORE any record is considered. A bundle whose bytes moved —',
      'a truncated copy, an edit, a transfer that did not finish — is refused whole, and nothing is',
      'imported from it.',
      '',
      'Importing the same bundle twice writes nothing the second time. A record this machine observed',
      'itself keeps its own first-hand origin even if somebody sends it back.',
      '',
      'An imported observation NEVER becomes a local one. "The other machine had gemma3 at 09:00" is',
      'what is learned, and it stays what is learned.',
    ],
  },
  {
    name: 'availability', effect: 'readOnly', positional: '',
    summary: 'which routes are observed available, on which machine, and from whose observation',
    options: [...ROOT,
      { name: 'machine', takesValue: true, summary: 'answer for this cmk1: key instead of this machine' },
      { name: 'route', takesValue: true, summary: 'only this route key (provider:modelID)' },
      { name: 'json', takesValue: false, summary: 'print the fleet view as JSON' }],
    detail: [
      'Every row says WHERE the observation was taken and whether this store made it or received it.',
      'A route available on another machine is shown as exactly that, and never as available here.',
    ],
  },
  {
    name: 'discovery-refresh', effect: 'invokesLocalTool', positional: '[<provider>…]',
    summary: 'observe what is available here now, persist the snapshot, and report what changed',
    options: [...ROOT,
      { name: 'json', takesValue: false, summary: 'print the snapshot and deltas as JSON' }],
    supportsDryRun: true,
    detail: [
      'Runs the same read-only probes `discover` runs and invokes no model. What it adds is PERSISTENCE:',
      'the snapshot is appended to the observation store and compared against the previous one taken on',
      'this machine, so a refresh can say what MOVED rather than only what is.',
      '',
      'A provider whose probe FAILED contributes no disappearance. "The tool did not answer" is reported',
      'as notRefreshed and its routes are carried forward untouched — it is never read as "the model is',
      'gone", which would stale every qualification that route has.',
    ],
  },
  {
    name: 'candidates', effect: 'readOnly', positional: '<capability>',
    summary: 'the routes whose evidence qualifies them for a capability — a candidate SET, never a winner',
    options: [...ROOT,
      { name: 'machine', takesValue: true, summary: 'the cmk1: key the work would run on. Defaults to this machine' },
      { name: 'tier', takesValue: true, summary: 'also require qualification at this structural tier (tier1|tier2|tier3)' },
      { name: 'min-context', takesValue: true, summary: 'minimum context window, in tokens' },
      { name: 'cost-policy', takesValue: true, summary: "'zero' (default) or 'metered' to include metered routes" },
      { name: 'local-only', takesValue: false, summary: 'only routes that run on this machine' },
    ...
    detail: [
      ...
      'or picks: choosing between two qualified routes is a scheduling decision this engine does not make.',
      '',
      'Routing policy crp2 (Pass 8) admits a route whose served-model identity cannot be independently',
      'verified — Codex and OpenCode — when it is discovered, available here, fresh, qualified for the',
      'capability, currently admitted, payable, unblocked, and the task did not ask for verified identity.',
      'Such a route is ALWAYS reported as admitted/unverifiable and never as verified.',
      '',
      '--require-verified-identity is how a provenance-sensitive task excludes them, per query, with the',
      'exclusion stated. --policy crp1 re-answers under the superseded Pass 6/7 rule, which admitted none',
      'of them at all; it is there so a historical decision can be read under the policy that made it.',
    ],
  },
  ...
  {
    name: 'develop', effect: 'spendsAllowance', positional: '<name>',
    summary: 'plan and run a development campaign (repository understanding, multi-file editing) — SENDS REAL REQUESTS',
    options: [...ROOT,
      { name: 'candidates', takesValue: true, summary: 'provider:model[:effort], comma-separated; each proven by `smoke` first' },
      { name: 'suites', takesValue: true, summary: 'development suites to plan; default every registered one' },
      { name: 'repeats', takesValue: true, summary: 'graded repeats each task needs before it counts as measured' },
      { name: 'label', takesValue: true, summary: 'human-readable label' },
      { name: 'max-attempts', takesValue: true, summary: 'stop after this many attempts; continue with develop-resume' },
      { name: 'synthetic', takesValue: false, summary: 'a synthetic candidate; no provider, process or network is touched' },
      { name: 'synthetic-script', takesValue: true, summary: 'what the synthetic candidate does to each task\'s workspace' },
      { name: 'yes', takesValue: false, summary: 'confirm a real run (non-interactive use)' }],
    supportsDryRun: true,
    detail: [
      'Every attempt runs in a fresh disposable copy of a sealed fixture repository under the OS temp',
      'directory, is read back through the path fence, graded from what the workspace held, and deleted.',
      'The candidate is given file tools and no shell. Only claudeCLI has a verified argument shape for',
      'that; every other provider is refused before anything is created, as is an unproven identity or',
      'a candidate the cost policy does not authorize.',
      '',
      '  --dry-run           print the plan — candidates, tasks, repeats, attempts, commit, machine and',
      '                      eligibility — and create nothing, write nothing, send nothing.',
      '  --yes               required for a real run. Without it the plan is printed and nothing runs.',
      '  --synthetic         exercise the whole runner with a candidate that contacts nothing. Given no',
      '                      --synthetic-script it does nothing and is graded as the empty attempt it is.',
      '',
      'A task counts as measured only when every one of its --repeats is graded. An attempt no model',
      'answered — transport, account, refusal — is excluded, never scored as a failure.',
    ],
  },
  {
    name: 'develop-resume', effect: 'spendsAllowance', positional: '<name>',
    summary: 'continue a development campaign from its ledger — SENDS REAL REQUESTS',
    options: [...ROOT,
      { name: 'max-attempts', takesValue: true, summary: 'stop after this many attempts' },
      { name: 'synthetic-script', takesValue: true, summary: 'the script a synthetic campaign was created with' },
      { name: 'yes', takesValue: false, summary: 'confirm a real run (non-interactive use)' }],
    supportsDryRun: true,
    detail: [
      'Runs only the task repeats with no terminal row, under the plan the campaign was created with. A',
      'recorded attempt is never re-run or rewritten, and a plan that no longer digests the same is refused.',
    ],
  },
  {
    name: 'develop-status', effect: 'readOnly', positional: '<name>',
    summary: 'task coverage, repeat completeness and grades of a development campaign',
