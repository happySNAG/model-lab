...
describe('the fixture runs only inside a disposable workspace', () => {
  ...
  it('is not the Cernum checkout, and is not any ancestor of it', async () => {
    ...
    // Resolved through its (still existing) parent: on macOS `os.tmpdir()` is `/var/...`, a symlink to
    // `/private/var/...`, and the workspace itself is already deleted when the attempt returns.
    const resolved = path.join(fs.realpathSync(path.dirname(outcome.workspaceRoot)), path.basename(outcome.workspaceRoot));
    expect(resolved.startsWith(root)).toBe(true);
    expect(fs.realpathSync(process.cwd()).startsWith(resolved)).toBe(false);
    expect(resolved).not.toBe(fs.realpathSync(process.cwd()));
  });
