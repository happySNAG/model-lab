...
const { defaultCurrency, maximumEntriesPerJournal } = require('./config.js');
...
function totalOf(journal) {
  if (journal.entries.length === 0) return { amountCents: 0, currency: defaultCurrency() };
  const currency = journal.entries[0].currency;
  for (const entry of journal.entries) {
    if (entry.currency !== currency) {
      throw new RangeError(`a journal that mixes ${currency} and ${entry.currency} has no total`);
    }
  }
  return { amountCents: journal.entries.reduce((total, entry) => total + entry.amountCents, 0), currency };
}
