...
| BENCHMARK | prose campaigns; development runner; workspace tiers 1–3 and discriminator-one through Claude, Codex, OpenCode and Ollama drivers, with continuation and exact cell selection | live OpenCode qualification (blocked on an account billing reading only a person can make — see `zero-cost-authorization.ts`); live Ollama qualification (blocked while the external volume holding the weights is not mounted) |
